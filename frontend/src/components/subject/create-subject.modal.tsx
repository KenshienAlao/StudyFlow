import { Dialog, DialogContent, DialogDescription, DialogHeader, DialogTitle, Input, Button } from "@/components/ui";
...
export function CreateSubjectModal({ isOpen, setIsOpen, isCreateSubjectPending, errorCreateSubject, handleCreateSubject }: CreateSubjectModalProps) {
    return (
        <Dialog open={isOpen} onOpenChange={(open) => !isCreateSubjectPending && setIsOpen(open)}>
                  <DialogContent className="max-w-md rounded-2xl border-border/60 bg-card p-6 shadow-2xl">
                    <DialogHeader>
                      <DialogTitle className="text-xl font-bold tracking-tight">Create New Subject</DialogTitle>
                      <DialogDescription className="text-sm text-muted-foreground">
                        Set up a custom classification directory for your focused track loops.
                      </DialogDescription>
                    </DialogHeader>
                    ...
                    <form onSubmit={handleCreateSubject} className="space-y-5 pt-2">
                      ...
                      <div>
                        ...
                        <Input
                          name="description"
                          placeholder="e.g., Covering wave functions, operators, and matrices"
                          disabled={isCreateSubjectPending}
                          className="h-10 border-border/80 focus:border-primary placeholder:text-muted-foreground/50"
                        />
                      </div>
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-3 bg-muted/40 border border-border/60 rounded-xl p-3">
                          ...
                          <div className="text-xs text-muted-foreground leading-normal">
                            Select a unique identifier hue to render card visualization indicators nicely.
                          </div>
                        </div>
                      </div>
