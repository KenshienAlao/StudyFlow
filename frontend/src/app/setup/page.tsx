...
import { SetupSchema } from "@/validation/profile.validation";
...
function PersonalStep({ step }: { step: number }) {
  return (
    <div
      data-step="1"
      className={`space-y-4 animate-fade-in ${step === 1 ? "block" : "hidden"}`}
    >
...
function DemographicsStep({ step }: { step: number }) {
  return (
    <div
      data-step="2"
      className={`space-y-4 animate-fade-in ${step === 2 ? "block" : "hidden"}`}
    >
      <div className="border-b border-border/60 pb-2 mb-4">
...
function PreferencesStep({
  ...
  step: number;
  setShowTerms: (s: boolean) => void;
}) {
  return (
    <div
      data-step="3"
      className={`space-y-4 animate-fade-in ${step === 3 ? "block" : "hidden"}`}
    >
      <div className="border-b border-border/60 pb-2 mb-4">
      ...
      <div className="space-y-4">
        ...
        <div className="bg-muted/40 border border-border/60 rounded-xl p-4 flex items-start gap-3 mt-4">
          <input
            ...
            required
            className="mt-1 rounded border-border text-primary focus:ring-primary cursor-pointer w-4 h-4 accent-primary"
          />
          <label
            htmlFor="agreedToTermsAndConditions"
            className="text-sm cursor-pointer text-muted-foreground leading-normal"
          >
            I agree to the
            <button
              type="button"
      ...
      <div className="w-full max-w-xl bg-card border border-border/80 rounded-2xl p-8 shadow-xl shadow-primary/5 transition-all">
        <div className="mb-8 text-center sm:text-left">
          ...
          <p className="text-sm text-muted-foreground">
            Let&apos;s get to know you better to personalize your study
            workspace.
          </p>
        </div>
        ...
        <div className="relative flex items-center justify-between w-full mb-10 px-4">
          ...
          <div className="relative z-10 flex flex-col items-center">
            ...
            <span
              className={`text-[11px] font-bold mt-1.5 transition-colors duration-300 ${step >= 1 ? "text-primary" : "text-muted-foreground"}`}
            >
              Personal
            </span>
          </div>
          ...
          <div className="relative z-10 flex flex-col items-center">
            ...
            <span
              className={`text-[11px] font-bold mt-1.5 transition-colors duration-300 ${step >= 2 ? "text-primary" : "text-muted-foreground"}`}
            >
              Details
            </span>
          </div>
          ...
          <div className="relative z-10 flex flex-col items-center">
            ...
            <span
              className={`text-[11px] font-bold mt-1.5 transition-colors duration-300 ${step >= 3 ? "text-primary" : "text-muted-foreground"}`}
            >
              Preferences
            </span>
          </div>
        ...
        {error && (
          <div className="mb-6 p-4 bg-destructive/10 border border-destructive/20 rounded-xl flex gap-3 items-center text-destructive text-sm animate-fade-in">
            ...
            <span className="font-medium">
              {error.message ||
                "An error occurred while saving your profile. Please check your network and try again."}
            </span>
          </div>
        )}
        ...
        <form ref={formRef} onSubmit={onSubmit} className="space-y-6">
          <fieldset
            disabled={isPending}
            className="space-y-6 disabled:opacity-85"
          >
            <PersonalStep step={step} />
            <DemographicsStep step={step} />
            <PreferencesStep step={step} setShowTerms={setShowTerms} />
          </fieldset>
        ...
        <Dialog open={showTerms} onOpenChange={setShowTerms}>
          <DialogContent className="max-w-xl max-h-[75vh] overflow-y-auto rounded-2xl border-border/60">
            <DialogHeader>
              ...
              <DialogDescription>
                Please review our basic usage protocols.
              </DialogDescription>
            </DialogHeader>
            <div className="space-y-4 text-sm text-muted-foreground leading-relaxed py-2">
              <p>
                1. By using StudyFlow, you agree to comply with our localized
                platform terms.
              </p>
              <p>
                2. We value data sovereignty and will protect your data metrics
                with industry standards.
              </p>
              <p>
                3. Do not exploit or systematically harvest our curated
                workspace tools.
              </p>
              <p>
                4. This is an active production ecosystem, please keep backup
                points for critical educational assets.
              </p>
            </div>
